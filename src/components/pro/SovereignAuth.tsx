      ...
      <div className="rounded-3xl border border-white/20 bg-card/60 backdrop-blur-xl p-8 max-w-sm w-full text-center space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground transition-all duration-300">
            ...
            {stage === 'scanning' && 'Decrypting Pure Alpha access...'}
            {stage === 'verified' && 'Identity confirmed. Welcome.'}
          </p>
        </div>

        {/* Retained the technical readout to keep the high-fidelity aesthetic */}
        <div className={`rounded-lg bg-muted/50 p-3 text-left space-y-1.5 transition-opacity duration-500 ${stage === 'verified' ? 'opacity-50' : 'opacity-100'}`}>
          <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
            <ShieldCheck className={`w-3 h-3 ${stage === 'verified' ? 'text-[hsl(142,71%,45%)]' : 'text-[hsl(28,80%,55%)]'}`} />
            Secure Handshake 
          </div>
          <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
            <Fingerprint className={`w-3 h-3 ${stage === 'verified' ? 'text-[hsl(142,71%,45%)]' : 'text-[hsl(178,42%,32%)]'}`} />
            Automated Biometric Pass
          </div>
          <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
            <Lock className={`w-3 h-3 ${stage === 'verified' ? 'text-[hsl(142,71%,45%)]' : 'text-[hsl(270,60%,50%)]'}`} />
            Accessing Pro Tiers
          </div>
        </div>
      </div>
