...
const CPMDashboard = ({ isMasked = false }: { isMasked?: boolean }) => {
  ...
  // --- 5-ROUND RSVP ENGINE ---
  const [rsvpPhase, setRsvpPhase] = useState<
    "IDLE" | "CALIBRATING" | "PRESENTING" | "MASK" | "RECALL" | "ROUND_COMPLETE" | "RESULT"
  >("IDLE");
  ...
  return (
    <>
      <GammaPhotosensitivityWarning
        ...
        onAcknowledge={() => {
          setGammaWarningOpen(false);
          triggerGammaSequence(true);
        }}
      />
      {isFlashing &&
        createPortal(
        ...
        <Tabs defaultValue="biometrics" className="w-full" style={{ transform: "translateZ(20px)" }}>
          ...
          <TabsContent value="gamma" className="space-y-6 focus-visible:outline-none">
            ...
            >
              ...
              <div
                className="flex items-center justify-between bg-card border-border p-5 rounded-2xl shadow-md"
              ...
              >
                ...
                <Switch
                  checked={gammaActive}
                  onCheckedChange={handleGammaToggle}
                  className="data-[state=checked]:bg-orange-500 shadow-sm"
                />
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="memory" className="space-y-6 focus-visible:outline-none">
            ...
            >
              <div className="p-5 border-b border-border flex items-center justify-between">
                ...
                {rsvpPhase !== "IDLE" && (
                  <Badge className="bg-orange-500 text-white font-black text-[9px] px-2.5 shadow-sm">
                    {cumulativeScore}
                  </Badge>
                )}
              </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 relative">
                ...
                {rsvpPhase === "RESULT" && (
                  <div
                    className="text-center space-y-10 animate-in zoom-in duration-500"
