...
const AppleHealthModal = ({ isOpen, onClose, onComplete, existingConnection, onDisconnect }: AppleHealthModalProps) => {
  ...
  const [justFinishedSync, setJustFinishedSync] = useState(false); // 🚨 State Lock
  ...
  }, [currentUserId]); // Inside src/components/AppleHealthModal.tsx
  ...
  useEffect(() => {
    (window as any).onHealthDataSyncComplete = async (serverResponse: any) => {
      ...
      setSyncCount(count); // Parse the processed_data array returned by apple-health-sync
      ...
      setIsConnecting(false); // The critical fix: Call onComplete AND onClose
    ...
    (hash: string) => {
      ...
      if (webkit?.messageHandlers?.syncHealthData) {
        ...
        webkit.messageHandlers.syncHealthData.postMessage({
          ...
          config: {
            endpoint: `https://zxyngqciipcvveigrzqt.supabase.co/functions/v1/idia-synapse?aca_hash=${hash}`,
  ...
  const handleFinalize = () => {
    onComplete(); // Tells parent to refresh state
