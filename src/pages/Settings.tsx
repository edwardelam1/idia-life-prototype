...
const VALID_TABS = ['idia-profile', 'appearance', 'privacy', 'notifications'] as const;

export default function Settings() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const [user, setUser] = useState<any>(null);
  const [activeTab, setActiveTab] = useState<string>('idia-profile');
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Scrollable content area — bottom safe-area inset for iOS home bar / Android nav */}
      <div
        className="flex-1 overflow-y-auto"
        style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 1rem)' }}
      >
        <div className="container max-w-4xl mx-auto px-2 py-3">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
