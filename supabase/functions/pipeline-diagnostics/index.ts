...
serve(async (req) => {
  ...
  try {
    ...
    const { data: healthData, error: healthError } = await supabase.rpc("check_pipeline_health");

    if (healthError) {
      console.error("Pipeline-Diagnostics: Error getting health data:", healthError);
      return new Response(JSON.stringify({ error: "Failed to get pipeline health", details: healthError }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const { data: recentStagedData, error: stagedError } = await supabase
      ...
      .select("id, pseudo_user_id, processed_at")
    ...
    const diagnostics = {
      ...
      status: {
        overall:
          processingEfficiency > 80 && rewardEfficiency > 80
            ? "healthy"
            : processingEfficiency > 50 && rewardEfficiency > 50
              ? "warning"
              : "critical",
    ...
    if (unprocessed > 0) {
      diagnostics.recommendations.push(
        `${unprocessed} raw health data entries need processing. Consider running backlog processor.`,
      );
    }
    if (unrewarded > 0) {
    ...
    if (rewardEfficiency < 80) {
      diagnostics.recommendations.push(
        "Reward processing efficiency is below optimal. Check process-staged-data function logs.",
      );
    }
