...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Check URL parameters first (Standard GET request)
    let code = url.searchParams.get("code");
    let state = url.searchParams.get("state"); // user_id
    let error = url.searchParams.get("error");

    // 2. Fallback to POST Form Data (Azure AD B2C form_post behavior)
    if (req.method === "POST") {
      try {
        ...
        console.warn("Could not parse form data from Ford callback payload", e);
      }
    }

    if (error) {
      console.error("Ford OAuth error:", error);
      return new Response("OAuth authorization failed: " + error, { status: 400, headers: corsHeaders });
    }
