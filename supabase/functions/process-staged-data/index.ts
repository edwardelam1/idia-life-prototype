...
serve(async (req) => {
  ...
  try {
    ...
    const { data: rewardResult, error: rewardError } = await supabase.functions.invoke("calculate-enhanced-rewards", {
      body: { staged_data_id },
    });
    ...
    const { error: creditError } = await supabase.functions.invoke("credit-user-wallet", {
      body: {
