...
- The lock overlay blocks all content (fixed, full screen, above the header/nav) and offers a single "Verify Identity" action. It also exposes a "Sign out" escape.
- Verification path: the WKWebView bridge `window.webkit.messageHandlers.triggerBiologicalCapture` (same probe used by the veto gate). On the native shell a failed or cancelled capture keeps the overlay up and allows retry. On web/preview, where no enclave bridge exists, the overlay shows a confirm button so the browser build is not permanently locked out.
